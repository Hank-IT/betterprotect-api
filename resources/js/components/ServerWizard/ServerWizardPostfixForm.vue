...
<script>
    export default {
        ...
        props: ['bus', 'server'],
        created() {
            this.bus.$on('server-wizard-submit-postfix', this.submit);
        },
        methods: {
            submit(props) {
                // Prevent submit
                if (this.submitted || ! this.postfix_feature_enabled) {
                    props.nextTab();

                    return;
                }

                axios.post('/server-wizard/' + this.server.id + '/postfix', this.form).then((response) => {
                    this.errors = {};
                    this.$notify({
